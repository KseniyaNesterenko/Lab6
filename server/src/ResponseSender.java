...
public class ResponseSender {
    ...
    private static final Logger responseSenderLogger = LogManager.getLogger(ResponseSender.class);
    ...
    public void send(String str, InetAddress receiverAddress, int receiverPort) throws IOException {
        ...
        if (byteArr.length > 8) {
            responseSenderLogger.warn("Размер пакета превышает допустимый. Разделить пакеты пока не представляется возможным");
            return;
        } else {
